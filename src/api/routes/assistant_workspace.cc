...
namespace routes {
    void init_assistant_workspace_routes(crow::SimpleApp &app) {
        ...
        CROW_ROUTE(app, "/assistant_workspace/<int>/<int>").methods("DELETE"_method)([](const crow::request &req, const int &assistant_id, const int &workspace_id){
            return controllers::delete_assistant_workspace(req, assistant_id, workspace_id);
        });

        CROW_ROUTE(app, "/assistant_workspace/<int>/<int>").methods("PATCH"_method)([](const crow::request &req, const int &assistant_id, const int &workspace_id){
            return controllers::update_assistant_workspace(req, assistant_id, workspace_id);
        });

        CROW_ROUTE(app, "/assistant_workspace/<int>/<int>").methods("GET"_method)([](const crow::request &req, const int &assistant_id, const int &workspace_id){
