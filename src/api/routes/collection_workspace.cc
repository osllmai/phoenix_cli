...
namespace routes {
    void init_collection_workspace_routes(crow::SimpleApp &app) {
        ...
        CROW_ROUTE(app, "/collection_workspace/<int>/<int>").methods("DELETE"_method)([](const crow::request &req, const int &collection_id, const int &workspace_id){
            return controllers::delete_collection_workspace(req, collection_id, workspace_id);
        });

        CROW_ROUTE(app, "/collection_workspace/<int>/<int>").methods("PATCH"_method)([](const crow::request &req, const int &collection_id, const int &workspace_id){
            return controllers::update_collection_workspace(req, collection_id, workspace_id);
        });

        CROW_ROUTE(app, "/collection_workspace/<int>/<int>").methods("GET"_method)([](const crow::request &req, const int &collection_id, const int &workspace_id){
