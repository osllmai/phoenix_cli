...
std::vector<llama_token> PhoenixTalkLlama::llama_tokenize(struct llama_context *ctx, const std::string &text,
                                                          bool add_bos) {
...
std::string PhoenixTalkLlama::llama_token_to_piece(const struct llama_context *ctx, llama_token token) {
...
bool PhoenixTalkLlama::whisper_params_parse(int argc, char **argv, whisper_params &params) {
    for (int i = 1; i < argc; i++) {
        std::string arg = argv[i];

        if (arg == "-h" || arg == "--help") {
            PhoenixTalkLlama::whisper_print_usage(argc, argv, params);
            exit(0);
...
void PhoenixTalkLlama::whisper_print_usage(int /*argc*/, char **argv, const whisper_params &params) {
    fprintf(stderr, "\n");
...
std::string PhoenixTalkLlama::transcribe(
        whisper_context *ctx,
...
std::vector<std::string> PhoenixTalkLlama::get_words(const std::string &txt) {
...
int PhoenixTalkLlama::phoenix_talk_llama(int argc, char **argv) {
    ...
    if (!path_session.empty()) {
        ...
        if (fp != NULL) {
            ...
            if (!llama_state_load_file(ctx_llama, path_session.c_str(), session_tokens.data(),
    ...
    while (is_running) {
...

        {
            ...
            if (::vad_simple(pcmf32_cur, WHISPER_SAMPLE_RATE, 1250, params.vad_thold, params.freq_thold,
                             params.print_energy) || force_speak) {
                ...
                while (true) {
...

                    {
                        ...
                        if (!path_session.empty() && need_to_save_session) {
                            ...
                            llama_state_save_file(ctx_llama, path_session.c_str(), session_tokens.data(),
                                                  session_tokens.size());
                        }
