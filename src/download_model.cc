...
size_t PhoenixDownloadModel::write_callback(void *contents, size_t size, size_t nmemb, void *userp) {
    ...
    outstream->write(static_cast<char *>(contents), total_size);
    return total_size;
}
...
int PhoenixDownloadModel::progress_callback(void *p, curl_off_t dltotal, curl_off_t dlnow, curl_off_t ultotal,
                                            curl_off_t ulnow) {
